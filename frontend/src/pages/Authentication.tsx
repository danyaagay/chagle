...
export default function () {
	...
	const callbackGoogle = async (codeResponse:any) => {
		const { code, scope, authuser } = codeResponse;
		const resp = await axios.get(`/auth/callback?code=${code}&scope=${scope}&authuser=${authuser}&prompt=none`);
		if (resp.status === 200) {
	...
	const handleSubmitGoogle = useGoogleLogin({
		onSuccess: codeResponse => {
			callbackGoogle(codeResponse);
					...
					<form onSubmit={form.onSubmit(handleSubmit)}>
						...
						{type !== 'signup' && (
							<>
								...
								<Button leftSection={<GoogleIcon />} onClick={() => handleSubmitGoogle()} variant="default" fullWidth mt="md" size="lg" fz="md" radius="md">
									Войти через Google
								</Button>
