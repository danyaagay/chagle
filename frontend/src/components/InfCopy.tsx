...
export default class InfiniteScroll extends Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Props) {
    console.log('update start', this.el.scrollTop);
    this._scrollableNode = this.getScrollableTarget();
    this.el = this.props.height
      ? this._infScroll
      : this._scrollableNode || window;
    ...
    console.log('update', this.el.scrollTop);
  ...
  parseThreshold(scrollThreshold: string | number) {
  ...
  isElementAtTop(target: HTMLElement, scrollThreshold: string | number = 0.8) {
    ...
    const threshold = this.parseThreshold(scrollThreshold);

    //Это отвечает за пороговую загрузку
    ...
    //Это загрузит только когда прокрутка полностью на верху
    return (
      target.scrollTop <=
